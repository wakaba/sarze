...
=item max => {custom => $non_negative_integer}  (Default: 0)
...
worker steps.  It is invoked with an argument: The worker process
state object of the worker process (See L</WORKER PROCESS STATE
OBJECT>).  The method should not throw.  The method should not return
